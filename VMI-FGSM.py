...
if __name__ == '__main__':
    ...
    parameters = {'attack': 'VMI-FGSM', **vars(args)}
    experiment.log_parameters(parameters)
    experiment.set_name("VMI-FGSM_"+args.model+"_"+args.target)
    ...
    for batch_ndx, (x_test, y_test) in enumerate(loader):
        ...
        print('Running VMI-FGSM attack on batch ', batch_ndx)
        attack = torchattacks.VMIFGSM(source_model, eps=args.eps, alpha=args.alpha, steps=args.steps, decay=args.decay,
        ...
        suc_rate = 1 - clean_accuracy(target_model, adv_images_VMI[correct_batch_indices,:,:,:], y_test[correct_batch_indices])
        ...
        if correct_batch_indices.size(0) != 0:
            ...
            for idx in correct_batch_indices:
                ...
                if torch.argmax(target_model(adv_images_VMI[idx].unsqueeze(0))) != y_test[idx]:
